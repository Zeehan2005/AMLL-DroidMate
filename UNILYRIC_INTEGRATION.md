...
### 数据变更
搜索结果仍会按之前方式检索歌曲 hash，但 `songId` 字段现在组合了 hash 和 proposal id，格式为 `"<hash>::<proposal>"`。内部歌词下载逻辑仍然只使用前半段 hash，而 UI 可以读取后半段的 proposal 以便显示或调试。函数 `getKugouLyrics` 会自动拆分该组合字符串。
